...
function buildScenario(label: string, amount: number, freeCash: number, projections: DecisionProjection[]): Scenario {
  ...
  return {
    label,
    amount,
    freeCashAfter: roundMoney(freeCash - amount),
    negativeHorizons: projected.filter((item) => item.balance < 0).length,
    projected,
  };
}
...
export function ScenarioComparison({ entityName, freeCash, projections, onAskAurelian, onUsePlan }: Props) {
  ...
  return (
    <section className="panel mt-4 overflow-hidden border-primary/20 p-4 sm:p-5">
      ...
      {ready ? (
        <div className={`mt-4 rounded-xl border p-3 text-xs leading-relaxed ${winner ? "border-primary/20 bg-primary/5" : "border-border bg-surface"}`}>
          {winner ? (
            <div className="flex items-start gap-2"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-primary" /><span><strong>{winner.label}</strong> preserva melhor o caixa pelos critérios atuais: menos horizontes negativos e maior Dinheiro Livre restante.</span></div>
          ) : (
            <div className="flex items-start gap-2"><GitCompareArrows className="mt-0.5 size-4 shrink-0 text-primary" /><span>Os dois planos têm impacto financeiro equivalente pelos critérios atuais.</span></div>
          )}
        </div>
      ) : null}
  ...
  return (
    <div className={`rounded-xl border bg-surface p-4 ${highlighted ? "border-primary/40" : "border-border"}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        {highlighted ? <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-medium text-primary">mais conservador</span> : null}
      </div>
      <label className="mt-3 block text-xs text-muted-foreground">Gasto hipotético hoje</label>
      <div className="mt-1 flex items-center rounded-lg border border-border bg-background px-3">
        <span className="text-sm text-muted-foreground">R$</span>
        <input inputMode="decimal" value={raw} onChange={(event) => onChange(event.target.value)} placeholder="5.000,00" className="h-10 min-w-0 flex-1 bg-transparent px-2 text-sm outline-none" />
      </div>

      {ready ? (
        <>
          <div className={`mt-3 rounded-lg border p-3 ${scenario.freeCashAfter < 0 ? "border-destructive/25 bg-destructive/5" : "border-border"}`}>
            <span className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Dinheiro Livre depois</span>
            <p className={`num mt-1 text-lg font-semibold ${scenario.freeCashAfter < 0 ? "text-destructive" : "text-success"}`}>{brl(scenario.freeCashAfter)}</p>
            {scenario.negativeHorizons > 0 ? <p className="mt-1 flex items-center gap-1 text-[10px] text-destructive"><TriangleAlert className="size-3" /> {scenario.negativeHorizons} horizonte(s) com saldo negativo</p> : <p className="mt-1 text-[10px] text-muted-foreground">nenhum horizonte negativo</p>}
          </div>
