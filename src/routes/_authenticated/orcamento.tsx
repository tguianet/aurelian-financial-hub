...
function Orcamento() {
  ...
  const expenseCategories = useMemo(
    () => selectableCategories(data.categories, "expense"),
    [data.categories],
  );
  ...
  const saveBudget = async () => {
    ...
    if (!selectedCategory) { toast.error("Selecione uma categoria de saída ativa."); return; }
    ...
    const { error } = await supabase.rpc("upsert_budget", {
      ...
      p_month: month,
      p_planned_amount: value,
    });
    setBusy(false);
    if (error) { toast.error(rpcErrorMessage(error, "Não foi possível salvar o orçamento.")); return; }
  ...
  const removeBudget = async (id: string) => {
    if (!canWrite) return;
    const { error } = await supabase.rpc("delete_budget", { p_id: id });
    if (error) { toast.error(rpcErrorMessage(error, "Não foi possível remover o orçamento.")); return; }
  ...
  return (
    <div>
            ...
            <Dialog open={open} onOpenChange={(next) => {
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <DialogFooter>
                  ...
                  <Button onClick={saveBudget} disabled={busy || expenseCategories.length === 0}>{busy ? "Salvando…" : "Salvar"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          ) : undefined
      ...
      {expenseCategories.length === 0 ? (
        <div className="mb-5 rounded-xl border border-primary/25 bg-primary/8 p-4 text-sm text-muted-foreground">
          Ainda não há categorias de saída neste espaço.
          {canWrite ? <> Abra <Link to="/categorias" className="font-medium text-primary underline-offset-2 hover:underline">Categorias</Link> para criar ou restaurar as categorias padrão.</> : null}
        </div>
      ) : null}
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-border/60 last:border-0 hover:bg-surface">
                ...
                <Td className="text-right">{canWrite ? <Button variant="ghost" size="icon" onClick={() => removeBudget(r.id)} title="Remover"><Trash2 className="size-4" /></Button> : null}</Td>
              </tr>
            ))}
