...
function Relatorios() {
  ...
  const rows = categoryBreakdown(data, entityId, toDate(from), toDate(to));
  ...
  return (
    <div>
      <PageHeader title="Relatórios" subtitle={`${entityName} · período selecionado`} />
      <DemoNotice />
                ...
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="p-8 text-center text-sm text-muted-foreground">
                      Nenhum lançamento liquidado no período.
                    </td>
                  </tr>
                ) : null}
              </tbody>
        ...
        <div className="panel p-5">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={rows.filter((r) => r.expense > 0)}
                  dataKey="expense"
                  nameKey="name"
                  innerRadius={55}
                  outerRadius={95}
      ...
      <div className="panel mt-5 p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[560px] text-sm">
            ...
            <tbody>
              {byEntity.map((r) => (
                <tr key={r.entity.id} className="border-b border-border/60 last:border-0">
                  <Td>{r.entity.name}</Td>
                  <Td className="num text-right text-success">{brl(r.income)}</Td>
                  <Td className="num text-right text-destructive">{brl(r.expense)}</Td>
                  <Td
