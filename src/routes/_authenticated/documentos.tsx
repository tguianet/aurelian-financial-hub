...
import { Download, File, FileSpreadsheet, Image as ImageIcon, RefreshCw, Sparkles, Stethoscope, Trash2 } from "lucide-react";
...
function Documentos() {
  ...
  const load = useCallback(async () => {
    ...
    const [catalog, storage] = await Promise.all([
      supabase
        ...
        .select("id, file_name, storage_path, status, created_at, size_bytes, interpretation_json, interpretation_error, transaction_id, credit_card_purchase_id")
    ...
    setLoading(false);
    if (catalog.error) { toast.error(catalog.error.message); return; }
  ...
  const interpret = async (doc: CatalogDoc, force = false) => {
    ...
    try {
      ...
      toast.success(result.source === "cached" ? "Sugestão já existente. Revise antes de lançar." : "Documento lido. Revise antes de confirmar.");
      void load();
    } catch (error) {
  ...
  const removeOrphan = async (path: string) => {
    ...
    if (error) { toast.error(error.message); return; }
    toast.success("Arquivo órfão excluído.");
    void load();
  };
  ...
  const diagnose = async () => {
    ...
    if (error) { toast.error(rpcErrorMessage(error, "Não foi possível diagnosticar.")); return; }
    const count = data?.length ?? 0;
    toast.message(count ? `${count} inconsistência(s) encontrada(s). Nada foi apagado.` : "Nenhuma inconsistência encontrada.");
  };

  const pending = docs.filter((doc) => doc.status !== "linked");
  const linked = docs.filter((doc) => doc.status === "linked");
  const pendingCount = pending.length + orphans.length;

  return (
    <div>
      <PageHeader
        title="Caixa de documentos"
        subtitle="Upload, interpretação por IA, revisão humana e lançamento confirmado."
        action={
      ...
      {review ? (
        <DocumentReviewDialog
          ...
          onConfirmed={() => { refresh(); void load(); }}
        />
      ) : null}

      <div className="panel p-4 md:p-5">
        <h2 className="text-sm font-semibold">Adicionar documento</h2>
        <p className="mt-1 text-xs text-muted-foreground">A IA só sugere. O lançamento só nasce depois de Confirmar lançamento.</p>
        {canWrite ? <QuickDocumentUpload /> : <p className="mt-3 text-sm text-muted-foreground">Seu acesso é somente leitura.</p>}
      </div>
      ...
      <div className="panel mt-5 p-4 md:p-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Enviado, processando, interpretado ou falhou. Órfãos no Storage podem ser recatalogados.</p>
          </div>
          <span className="rounded-full border border-border px-2.5 py-1 text-xs text-muted-foreground">{pendingCount}</span>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            ...
            return (
              <div key={doc.id} className="rounded-xl border border-border bg-surface p-4">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <Button variant="outline" size="sm" className="gap-2" onClick={() => void openFile(doc.storage_path)}>
                    <Download className="size-4" /> Abrir
                  </Button>
                  {canWrite ? (
                    <Button variant="ghost" size="sm" className="gap-2 text-destructive hover:text-destructive" onClick={() => void remove(doc)}>
                      <Trash2 className="size-4" /> {doc.status === "linked" ? "Arquivar" : "Excluir"}
                    </Button>
                  ) : null}
                </div>
                {canWrite && (doc.status === "uploaded" || doc.status === "failed") ? (
                  <Button variant="outline" size="sm" className="mt-2 w-full gap-2" disabled={busy} onClick={() => void interpret(doc)}>
                    <Sparkles className="size-4" /> {busy ? "Lendo…" : doc.status === "failed" ? "Tentar novamente" : "Interpretar"}
                  </Button>
                ) : null}
                {doc.status === "processing" ? (
                  <p className="mt-2 text-center text-[11px] text-muted-foreground">Processando com IA…</p>
                ) : null}
                {canWrite && doc.status === "interpreted" ? (
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    <Button size="sm" onClick={() => openReview(doc)}>Revisar / Confirmar</Button>
                    <Button variant="outline" size="sm" disabled={busy} onClick={() => void interpret(doc, true)}>Reprocessar</Button>
                  </div>
                ) : null}
              </div>
            );
          ...
          {orphans.map((file) => {
            ...
            return (
              <div key={file.path} className="rounded-xl border border-dashed border-border bg-surface p-4">
                <div className="flex items-start gap-3">
                  <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary"><Icon className="size-5" /></span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium" title={file.name}>{file.name}</p>
                    <p className="mt-1 text-[11px] text-muted-foreground">Órfão no Storage · sem metadata</p>
                  </div>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2">
                  ...
                  {canWrite ? (
                    <Button variant="ghost" size="sm" className="gap-2 text-destructive hover:text-destructive" onClick={() => void removeOrphan(file.path)}>
                      <Trash2 className="size-4" /> Excluir
                    </Button>
                  ) : null}
                </div>
                {canWrite ? (
                  <Button variant="outline" size="sm" className="mt-2 w-full" onClick={() => void recatalog(file.path, file.name)}>
                    Recatalogar
                  </Button>
                ) : null}
              </div>
      ...
      {linked.length ? (
        <div className="panel mt-5 p-4 md:p-5">
          <h2 className="text-sm font-semibold">Lançados</h2>
          <p className="mb-4 text-xs text-muted-foreground">Documento vinculado. Arquivar não apaga o lançamento.</p>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {linked.map((doc) => {
              const Icon = iconFor(doc.file_name);
              return (
                <div key={doc.id} className="rounded-xl border border-border bg-surface p-4">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    ...
                    {canWrite ? (
                      <Button variant="ghost" size="sm" onClick={() => void remove(doc)}>Arquivar</Button>
                    ) : null}
                  </div>
                </div>
              );
